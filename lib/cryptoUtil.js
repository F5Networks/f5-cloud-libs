    ...
    encrypt: function encrypt(publicKeyDataOrFile, data) {
        ...
        publicKeyPromise
            .then((publicKey) => {
                ...
                try {
                    encrypted = crypto.publicEncrypt(publicKey, createBufferFrom(data));
 ...
 */
function createBufferFrom(data, encoding) {
    if (typeof data !== 'string') {
    ...
    if (nodeVersion.split('.')[0] === '4' && (util.versionCompare(nodeVersion, '4.5.0') > -1)) {
        useBufferFrom = true;
    } else if (util.versionCompare(nodeVersion, '5.10.0') > -1) {
        useBufferFrom = true;
    } else {
        useBufferFrom = false;
    }
