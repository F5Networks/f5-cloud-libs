...
module.exports = {
    ...
    checkPasswordStrength(length, password) {
        ...
        if (password.length >= length &&
            ...
            sc && sc.length >= specialMinCount) {
                ...
                deferred.reject(new Error('Password weak'));
            }
            return deferred.promise;
    },
    ...
    checkPasswordAll(length, password) {
        const deferred = q.defer();
        q.all([checkPasswordStrength(length, password), checkPasswordCrack(password)])
            .then(() => {
                deferred.resolve(password);
            })
            .catch((err) => {
                deferred.reject(new Error('Password sucks, generating a new one'));
            });
            return deferred.promise;
    },
