...
function BigIq52LicenseProvider(bigIp, options) {
    const version = options ? options.version : '5.2.0';
 ...
 */
BigIq52LicenseProvider.prototype.revoke = function revoke(bigIqControl, poolName, instance) {
    let poolUuid;
                        ...
                        getMembersForKey(bigIqControl, poolUuid, license.licenseState.registrationKey)
                            .then((membersForKey) => {
                                ...
                                for (let i = 0; i < members.length; i++) {
                                    if (members[i].deviceName === instance.hostname) {
                                        found = true;
                                        deferred.resolve(
                                            {
...
function tryRegKey(bigIqControl, bigIpMgmtAddress, bigIpMgmtPort, poolUuid, regKey) {
    ...
    // Even though this is the 5.2 license provider, the same API exists on newer versions.
    // The only change is that instead of putting the port in the deviceAddress, it is a
    // separate field
    let deviceAddress;
    let port;

    if (util.versionCompare(this.version, '5.2.0') > 0) {
