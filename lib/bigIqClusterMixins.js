...
const bigIqClusterMixins = {
    ...
    waitForPeerReady(remoteBigIp, failoverPeerIp, failoverPeerUsername, failoverPeerPassword, retryOptions) {
        const func = function () {
            return q()
                .then(() => {
                    // Determine if user is enabled on failover peer, using a very long retry
                    // so we do not lock out the user on the failover peer
                    ...
                    return icontrol.create('/shared/authn/login',
                        {
                            username: failoverPeerUsername,
                            password: failoverPeerPassword
                        });
                })
                .then(() => {
                ...
                .then(() => {
                    // Check to see if secondary BIG-IQ is configured for clustering
                    return remoteBigIp.list('/shared/identified-devices/config/discovery');
                })
                .then((response) => {
                    if (response && response.discoveryAddress === failoverPeerIp) {
