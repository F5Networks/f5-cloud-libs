 ...
 * @returns {Promise} A promise which will be resolved when processing is complete.
 */
AutoscaleProvider.prototype.revokeLicenses = function(instances) {
    var promises = [];

    if (instances.length > 0) {
        if (!this.clOptions.licensePool) {
            return q.reject('Can only revoke licenses retrieved from BIG-IQ');
        }
        ...
        .then(function() {
            instances.forEach(function(instance) {
                promises.push(bigIq.revokeLicense(this.clOptons.licensePoolName, instance.mgmtIp, this.clOptions.bigIpMgmtPort));
            }.bind(this));
