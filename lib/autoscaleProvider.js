...
/**
 * Gets info on what each instance thinks the master status is
 *
 * Info is retrieval is cloud specific. Likely either from the cloud infrastructure
 * itself, stored info that we have in a database, or both.
 *
 * @abstract
 *
 * @param {String[]}  instanceIds - Array of instanceIds for which to get the master status
 * @returns {Promise} A promise which will be resolved with a dictionary of master
 *                    statuses keyed by instance ID. Each status value should be:
 *
 *                    {
 ...
 *
 */
AutoscaleProvider.prototype.getMasterStatus = function(instanceIds) {
    throw new Error("Unimplemented abstract method AutoscaleProvider.getMasterStatus", instanceIds);
};

/**
 * Returns the credentials for the given instanceId if known
 *
 * @param {String} instanceId - Instance ID of the instance for which to return credentials
 *
 * @returns {Promise} A promise which will be resolved with the credentials in the form
 *
 *                    {
 *                        username: <admin_user>,
 *                        password: <admin_password>
 *                    }
 */
AutoscaleProvider.getCredentials = function(instanceId) {
    this.logger.debug("No override for AutoscaleProvider.getCredentials", instanceId);
    return q();
};
