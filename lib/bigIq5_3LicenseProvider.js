...
*/
function BigIq5_3LicenseProvider(bigIpCore, options) {
    options = options || {};
    if (options.logger) {
...
BigIq5_3LicenseProvider.prototype.getUnmanagedDeviceLicense = function(bigIqControl, poolName, bigIpMgmtAddress, bigIpMgmtPort) {
    ...
    logger.debug('Licensing from pool', poolName);
    ...
    return bigIqControl.create(
        ...
        .then(function(response) {
            ...
            var isLicensed = function() {
                return bigIqControl.list(LICENSE_PATH + taskId)
                    .then(function(response) {
                        ...
                        logger.verbose('Current licensing task status:', status);
                        if (status === 'FINISHED') {
                            return q(
                                {
                                    success: true
            ...
            util.tryUntil(this, {maxRetries: 40, retryIntervalMs: 5000}, isLicensed)
                .then(function(response) {
                    if (response.success) {
                        logger.info("Successfully licensed");
                        return q();
                    }
                    else {
                        logger.info("Licensing failed", response.errorMessage);
                        return q.reject(new Error(response.errorMessage));
                    }
                }.bind(this))
                .catch(function(err) {
                    logger.info("Failed to license", err);
