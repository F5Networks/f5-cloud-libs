 ...
 */
BigIq50LicenseProvider.prototype.revoke = function revoke(bigIqControl, poolName, instance) {
    let poolUuid;
    ...
    return getPoolUuid(bigIqControl, poolName)
        ...
        .then((response) => {
            ...
            for (let i = 0; i < licenses.length; i++) {
                if (licenses[i].deviceName && licenses[i].deviceName === instance.hostname) {
                    license = licenses[i];
                    break;
                }
            }
            ...
            return q.reject(new Error('no license found for host:', instance.hostname));
        });
};
