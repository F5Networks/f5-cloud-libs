...
module.exports = {
     ...
     */
    generateAndInstallKeyPair: function(publicKeyDirectory, publicKeyOutFile, privateKeyFolder, privateKeyName, options) {
        ...
        return this.privateKeyExists(privateKeyFolder, privateKeyName)
            .then(function(hasPrivateKey) {
                if (hasPrivateKey && !options.force) {
                    logger.debug('Private key already exists');
                    return q();
                }
                else {
                    if (hasPrivateKey) {
                        logger.debug("Privage key exists. Regenerating.");
                    }
                    else {
    ...
    privateKeyExists: function(folder, name) {
