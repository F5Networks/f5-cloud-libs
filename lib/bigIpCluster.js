...
function BigIpCluster(bigIpCore, options) {
    ...
    loggerOptions = options ? options.loggerOptions : undefined;
...
BigIpCluster.prototype.addToTrust = function addToTrust(
    ...
    retry.continueOnErrorMessage = 'remoteSender';
...
BigIpCluster.prototype.createDeviceGroup = function createDeviceGroup(
    ...
    const func = function () {
        return this.core.ready()
            ...
            .then((response) => {
                if (response === false) {
                    groupSettings.name = deviceGroup;
                    groupSettings.devices = names || [];
                    groupSettings.type = type;

                    return this.core.create(DEVICE_GROUP_PATH, groupSettings, undefined, util.NO_RETRY);
                }

                // If the device group exists, re-apply group settings in case they've been updated
                return this.core.modify(
                    ...
                    .then(() => {
                        // Check that the requested devices are in the device group
 ...
 * @param {String}  remoteHost                  - Managemnt IP for the remote device.
 * @param {String}  remoteUser                  - Remote device admin user name.
 * @param {String}  remotePassword              - Remote device admin user password.
 * @param {Boolean} isLocal                     - Whether the device group is defined locally or if
 *                                                we are joining one on a remote host.
 * @param {Object}  [options]                   - Optional arguments.
 * @param {Number}  [options.remotePort]        - Remote device port to connect to. Default the
 *                                                port of this device instance.
 ...
 *                                                Default 10000.
 * @param {String[]} [options.syncCompDevices]  - List of device names that should be connected.
 *                                                If defined, sync complete failures are ignored
 *                                                when device trust can not sync due to disconnected
 *                                                devices that are not in this list. Default is to
 *                                                leave undefined and fail on all sync complete failures.
...
BigIpCluster.prototype.joinCluster = function joinCluster(
        ...
        const func = function () {
            ...
            return this.core.ready()
                ...
                .then((response) => {
                    ...
                    promises = [
                        this.core.deviceState(localHostname, util.NO_RETRY),
                        remoteBigIp.deviceState(remoteHostname, util.NO_RETRY)
                    ];
    ...
    const processJoin = function processJoin() {
        return remoteBigIp.init(
            ...
            remotePassword,
            {
                ...
                product: normalizedOptions.product
            }
        )
            .then(() => {
            ...
            .then((response) => {
                ...
                if (!isLocal) {
                    // We may get back just one device or an array of devices
                    const normalizedResponse = Array.isArray(response) ? response : [response];
                    const device = normalizedResponse.find((dev) => {
                        return dev.selfDevice === 'true';
                    });

                    if (typeof device === 'undefined') {
                        return q.reject(new Error('Self device could not be found to set local hostname'));
                    }
                            ...
                            if (err && err.recommendedAction) {
                                ...
                                if (err.recommendedAction.sync) {
                                    ...
                                    clusteringBigIp.sync('to-group', recommendedGroup, true, util.NO_RETRY)
                                        .then(() => {
                                            return clusteringBigIp.syncComplete(
                                                SYNC_COMPLETE_RETRY,
                                                {
                                                    connectedDevices: normalizedOptions.syncCompDevices
                                                }
                                            );
                                        })
                                        .then(() => {
            ...
            .catch((err) => {
                this.logger.info(`join cluster failed: ${err.message ? err.message : err}`);
                return q.reject(err);
            });
    }.bind(this);
    ...
    const ctorOptions = {};
    if (loggerOptions) {
        ctorOptions.loggerOptions = loggerOptions;
    } else {
        ctorOptions.logger = this.logger;
    }
    remoteBigIp = new BigIp(ctorOptions);
 ...
 * @param {Integer}  [retryOptions.retryIntervalMs] - Milliseconds between retries. Default 10000.
 * @param {Object}   [options]                      - Optional arguments.
 * @param {String[]} [options.connectedDevices]     - List of device names that should be connected.
 *                                                    If defined, sync complete failures are ignored
 *                                                    when device trust can not sync due to disconnected
 *                                                    devices that are not in this list. Default is to
 *                                                    leave undefined and fail on all sync complete failures.
 ...
 */
BigIpCluster.prototype.syncComplete = function syncComplete(retryOptions, options) {
    const retry = retryOptions || util.DEFAULT_RETRY;
    const opts = options || {};

    /**
     * Returns a promise that resolves true if at least one device is disconnected
     * and all supplied devices in the list are connected, otherwise it resolves false.
     */
    const isSyncDisconnectFailure = function isSyncDisconnectFailure(deviceNames) {
        let localHostname = '';
        ...
        return this.core.deviceInfo()
            .then((deviceInfo) => {
                localHostname = deviceInfo.hostname;
                return this.getCmSyncStatus();
            })
            .then((cmSyncStatus) => {
                if (cmSyncStatus.disconnected.length === 0) {
                    return false;
                }

                cmSyncStatus.connected.push(localHostname);
                return deviceNames.every((device) => {
                    return cmSyncStatus.connected.indexOf(device) >= 0;
                });
            });
    }.bind(this);
    ...
    return util.tryUntil(this, retry, func)
        .catch((err) => {
            if (err && err.recommendedAction && err.recommendedAction.sync === 'device_trust_group') {
                return isSyncDisconnectFailure(opts.connectedDevices)
                    .then((skipFail) => {
                        if (skipFail) {
                            return q.resolve();
                        }
