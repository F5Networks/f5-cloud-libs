...
BigIpCluster.prototype.joinCluster = function(deviceGroup, remoteHost, remoteUser, remotePassword, isLocal, options) {
    ...
    var checkClusterReadiness = function(deviceGroup) {
                    ...
                    promises = [
                        {
                            promise: this.list,
                            arguments: [
                                '/tm/cm/device/~Common~' + localHostname,
                                undefined,
                                util.NO_RETRY
                            ]
                        },
                        {
                            promise: remoteBigIp.list,
                            arguments: [
                                '/tm/cm/device/~Common~' + remoteHostname,
                                undefined,
                                util.NO_RETRY
                            ]
                        }
        ...
        return util.tryUntil(this, {maxRetries: 240, retryIntervalMs: 10000}, func);
    }.bind(this);
...
this.logger.info('SYNCING THIS', this.core.user, this.core.password);
this.logger.info('SYNCING REMOTE', remoteBigIp.user, remoteBigIp.password);
...
BigIpCluster.prototype.syncComplete = function(retryOptions) {
    ...
    var func = function() {
        var deferred = q.defer();
this.logger.info('CHECKING READY', this.core.user, this.core.password);
        this.core.ready()
            .then(function() {
