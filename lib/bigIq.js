 ...
 * @param {Number}  [options.port]              - Port of management port. Default 443.
...
BigIq.prototype.init = function init(host, user, passwordOrUri, options) {
    ...
    this.host = host ? host.trim() : 'localhost';
    this.user = user ? user.trim() : null;
    ...
    const authnOptions = {
        ...
        authProvider: this.initOptions.authProvider,
        port: this.initOptions.port
    };
    ...
    return authn.authenticate(this.host, this.user, passwordOrUri, authnOptions)
        .then((icontrol) => {
            ...
            return util.tryUntil(this, util.MEDIUM_RETRY, getVersion);
        })
        .then((response) => {
...
function getLicenseProvider(poolName, options) {
    ...
    if (methodOptions.autoApiType) {
        return getApiType.call(this, methodOptions)
            .then((apiType) => {
...
function getApiType(options) {
    const apiType = options.noUnreachable ?
