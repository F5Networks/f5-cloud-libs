 ...
 * @param {String}  [options.tenant]         - tenant parameter for CLPv2 licensing. Default none.
...
function licenseFromPool(bigIqControl, poolName, bigIpMgmtAddress, options) {
    ...
    // get our mac address
    return this.bigIp.getManagementMac()
        .then((response) => {
            macAddress = response;
        ...
        .then((deviceInfo) => {
            ...
            if (tenant) {
                tenantValue = `${tenant},
                mgmtAddress=${deviceInfo.managementAddress},hostname=${deviceInfo.hostname}`;
            } else {
                tenantValue = `mgmtAddress=${deviceInfo.managementAddress},hostname=${deviceInfo.hostname}`;
            }
            const createLicenseTask = function () {
                return bigIqControl.create(
                    LICENSE_PATH,
                    {
                        hypervisor,
            ...
            return util.tryUntil(this, util.MEDIUM_RETRY, createLicenseTask);
        })
        .then((response) => {
