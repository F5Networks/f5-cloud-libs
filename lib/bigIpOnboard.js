 ...
 *
 */
BigIpOnboard.prototype.licenseViaBigIq = function(host, user, password, poolName, bigIpMgmtAddress) {

    var deferred = q.defer();
    var IControl = require('./iControl');
    // this.bigIqControl is used for testing
    var bigIqControl = this.bigIqControl || new IControl(
        {
            host: host.trim(),
            user: user.trim(),
            ...
            if (response) {
                ...
                if (state === 'LICENSED') {
                    ...
                    util.tryUntil(this, {maxRetries: 40, retryIntervalMs: 5000}, isLicensed)
                        ...
                        .catch(function() {
                            deferred.reject('Giving up on licensing via BIG-IQ.');
                        });
                }
            }
        }.bind(this))
        ...
        .then(function(response) {
            ...
            if (currentUser !== -1) {
                ...
                if (user === this.core.user) {
                    this.core.init(this.core.host, this.core.user, password, this.core.port, options);
                }
            ...
            .then(function() {
                ...
                promise
                    .then(function(response) {
                        ...
                        // re-initialize the bigIp core
                        if (user === this.user) {
                            this.core.init(this.core.host, this.core.user, newPassword, {port: this.core.port});
                        }
        ...
        return this.core.ready()
            ...
            .then(function() {
                if (!noInit) {
                    ...
                    this.core.init(this.core.host, this.core.user, this.core.password, {port: sslPort});
                }
