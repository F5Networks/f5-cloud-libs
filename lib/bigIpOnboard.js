...
BigIpOnboard.prototype.hostname = function hostname(name, retryOptions) {
    ...
    const func = function () {
        return this.core.ready()
            ...
            .then((response) => {
                const device = response.find((dev) => {
                    return dev.selfDevice === 'true';
                });

                if (typeof device === 'undefined') {
                    return q.reject(new Error('Self device could not be found to set new hostname'));
                }
...
BigIpOnboard.prototype.license = function license(options, retryOptions) {
    ...
    retry.continueOnErrorMessage = /remoteSender:127.0.0.1/;
    ...
    const func = function () {
        ...
        return this.core.ready()
            .then(() => {
                return this.core.list('/tm/shared/licensing/registration', undefined, util.MEDIUM_RETRY);
            })
            .then((response) => {
                if (
                    (response.usage
 ...
 * @param {String}  [options.authProvider]      - Name of the auth provider to use. Default is tmos.
 ...
 * @param {String}  [options.bigIqMgmtPort]     - Port of BIG-IQ management port. Default is 443.
...
BigIpOnboard.prototype.licenseViaBigIq = function licenseViaBigIq(
    ...
    const authProvider = options ? options.authProvider : undefined;
    const bigIpMgmtPort = options ? options.bigIpMgmtPort : undefined;
    const bigIqMgmtPort = options ? options.bigIqMgmtPort : undefined;
 ...
 *                    is complete or rejected if an error occurs.
 */
BigIpOnboard.prototype.revokeLicense = function revokeLicense(retryOptions) {
    const revokeLicenseRetryOptions = {
        maxRetries: 5,
        retryIntervalMs: 10000
    };

    const retry = Object.assign({}, revokeLicenseRetryOptions, retryOptions);
    retry.continueOnErrorMessage = /remoteSender:127.0.0.1/;
    ...
    const func = function () {
        return this.core.ready()
            ...
            .then((regResponse) => {
                if (!regResponse.registrationKey) {
                    return 'No license to revoke. Skipping.';
                }
                if (regResponse.usage && regResponse.usage === 'Revoked License') {
                    return 'License is already revoked. Skipping.';
                }

                const licenseBody = {
                    command: 'revoke'
                };
                // If we are going to revoke, we first need to save sys config, otherwise
                // we lose any mods we have made but not yet saved
                return this.core.save()
                    .then(() => {
                        return this.core.create('/tm/sys/license', licenseBody, undefined, util.NO_RETRY);
                    ...
                    .then((revokeResponse) => {
                        const message = revokeResponse.commandResult.trim();
                        // Expect a different success message depending on the version of the BIG-IP
                        if (
                            message.indexOf('License successfully revoked') === -1
                            && message.indexOf('New license installed') === -1
                        ) {
                            return q.reject(new Error(message));
                        }
                        return message;
 ...
 * @param {Boolean} [options.useTransaction] - Uses transaction to provision all requested modules
    ...
    if (modulesToProvision.length > 0) {
        return this.core.ready()
            ...
            .then((response) => {
                ...
                const commands = modulesToProvision
                    .filter((module) => {
                        return provisionSettings[module] !== currentProvisioning[module];
                    })
                    .map((module) => {
                        return {
                            method: 'modify',
                            path: `${PROVISION_PATH}${module}`,
                            body: { level: provisionSettings[module] }
                        };
                    });

                if (methodOptions.useTransaction && commands.length > 0) {
                    provisioningCommands.push({
                        promise: this.core.transaction,
                        arguments: [commands]
                    });
                } else {
 ...
 */
BigIpOnboard.prototype.installIlxPackage = function installIlxPackage(packageUri, retryOptions) {
    const retry = retryOptions || util.DEFAULT_RETRY;
    ...
    const func = function () {
        return this.core.runTask(
            ...
            .catch((err) => {
                // If the ILX package is already installed, log it, but continue running onboard, don't reject
                if (err && err.message && err.message.indexOf('already installed') > -1) {
                    this.logger.warn(`Received task error, but continuing processing. ${err.message}`);
                    return q.resolve();
                }
                return q.reject(err);
            });
                ...
                if (defaultsChanged) {
                    return this.core.modify(
                        ...
                        .catch((err) => {
                            if (
                                err.message
                                && err.message.indexOf('eXtremeDB - unknown error code: 8') > -1
                            ) {
                                this.logger.debug(err.message.trim());
                                return q(`Unable to add port "${port}" to self allow defaults`);
                            }
                            return q.reject(err);
                        });
                }
