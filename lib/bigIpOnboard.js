...
BigIpOnboard.prototype.license = function(options, retryOptions) {
    ...
    var func = function() {
        ...
        return this.core.ready()
            ...
            .then(function(response) {
                if (response.registrationKey && !options.addOnKeys && response.registrationKey === options.registrationKey) {
 ...
 * @param {String} [role] - Role for user. Only valid when creating user. Default admin.
 *
 * @returns {Promise} A promise which is resolved when the user has been updated
 *                    or rejected if an error occurs.

 */
BigIpOnboard.prototype.updateUser = function(user, password, role) {
    return this.core.ready()
        .then(function() {
        ...
        .then(function(response) {
            ...
            if (currentUser !== -1) {
                ...
                if (user === this.user) {
                    this.core.init(this.core.host, this.core.user, password, this.core.port);
                }
            ...
            .then(function() {
                ...
                promise
                    .then(function(response) {
                        ...
                        // re-initialize the bigIp core
                        if (user === this.user) {
                            this.core.init(this.core.host, this.core.user, newPassword, this.core.port);
                        }
        ...
        return this.core.ready()
            ...
            .then(function() {
                if (!noInit) {
                    ...
                    this.core.init(this.core.host, this.core.user, this.core.password, sslPort);
                }
