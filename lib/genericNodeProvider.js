...
GenericNodeProvider.prototype.init = function init(providerOptions, options) {
    ...
    this.propertyPaths = {};
    ['propertyPathId', 'propertyPathIpPrivate', 'propertyPathIpPublic'].forEach((key) => {
        const val = this.providerOptions[key];
        if (typeof val === 'string') {
            let newKey = key.split('propertyPath')[1];
            newKey = newKey.charAt(0).toLowerCase() + newKey.slice(1);
            this.propertyPaths[newKey] = val.split('.');
        }
    });
    ...
    return cloudUtil.getDataFromUrl(uri, options)
        .then((data) => {
            ...
            for (let i = 0; i < resData.length; i++) {
                const node = { ip: {} };
                node.id = getDataFromPropPath(this.propertyPaths.id, resData[i]);
                node.ip.private = getDataFromPropPath(this.propertyPaths.ipPrivate, resData[i]);
                node.ip.public = getDataFromPropPath(this.propertyPaths.ipPublic, resData[i]);
