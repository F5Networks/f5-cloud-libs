 ...
 *                    elected master.
 */
CloudProvider.prototype.electMaster = function electMaster(instances) {
    throw new Error('Unimplemented abstract method CloudProvider.electMaster', instances);
};
 ...
 *                    }
 */
CloudProvider.prototype.getMasterCredentials = function getMasterCredentials(mgmtIp, mgmtPort) {
    if (!this.hasFeature(CloudProvider.FEATURE_MESSAGING)) {
        throw new Error(
            'Unimplemented abstract method CloudProvider.getMasterCredentials',
            mgmtIp,
            mgmtPort
        );
    } else {
        this.logger.debug('No override for CloudProvider.getMasterCredentials');
        return q(true);
    }
};
 ...
 *                    is complete
 */
CloudProvider.prototype.putMasterCredentials = function putMasterCredentials() {
    this.logger.debug('No override for CloudProvider.putMasterCredentials');
    return q();
};
 ...
 *
 */
CloudProvider.prototype.getMasterStatus = function getMasterStatus() {
    this.logger.debug('No override for CloudProvider.getMasterStatus');
    return q();
};
 ...
 */
CloudProvider.prototype.isValidMaster = function isValidMaster(instanceId, instances) {
    this.logger.debug('No override for CloudProvider.isValidMaster', instanceId, instances);
    return q(true);
};
 ...
 */
CloudProvider.prototype.masterElected = function masterElected(instanceId) {
    this.logger.debug('No override for CloudProvider.masterElected', instanceId);
    return q();
};

/**
 * Called after a master has been elected.
 *
 * In some cloud environments, instances running a master should be tagged through
 * cloud provider specific tagging. Override this method for cloud providers that support
 * instance tagging for instances running masters.
 *
 * @param {String} instanceId - Instance ID that was elected master.
 * @param {Object} instances - Dictionary of instances as returned by getInstances
 *
 * @returns {Promise} A promise which will be resolved when processing is complete.
 */
CloudProvider.prototype.tagMasterInstance = function tagMasterInstance(instanceId, instances) {
    this.logger.debug('No override for CloudProvider.tagMasterInstance', instanceId, instances);
    return q();
};
 ...
 * @returns {Promise} A promise which will be resolved when processing is complete.
 */
CloudProvider.prototype.masterInvalidated = function masterInvalidated(instanceId, instances) {
    this.logger.debug('No override for CloudProvider.masterInvalidated', instanceId, instances);
    return q();
};
