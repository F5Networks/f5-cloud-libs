...
function IControl(options) {
    ...
    this.basePath = opts.basePath || getBasePath.call(this, opts);
    this.strict = typeof opts.strict !== 'undefined' ? opts.strict : true;

    if (this.host !== 'localhost' || this.port !== 8100) {
...
IControl.prototype.request = function request(method, path, body, options, existingDeferred) {
    ...
    const responseHandler = noWait ? undefined : (response) => {
        ...
        response.on('end', () => {
            ...
            if (response.statusCode >= 300) {
                if (response.statusCode === 401 && this.authToken && !noRefresh) {
                    const retryOptions = {};
                    Object.assign(retryOptions, options);
                    retryOptions.noRefresh = true;

                    // BIG-IQ requests using Token Auth are required to originate from the same IP address.
                    // Requests from a client may originate from multiple IP addresses (ex: SNAT pools).
                    // Fall back to Basic Auth if BIG-IQ reports the token was issued to another client.
                    const invalidClient =
                        'Tokens are only valid to be used by the client of which they were issued.';
                    if (parsedResponse.message && parsedResponse.message.indexOf(invalidClient) > -1) {
                        this.logger.debug('auth token authentication failed, trying basic auth');
    ...
    const httpRequest = getRequest.call(this, requestOptions, responseHandler);
...
function getBasePath(options) {
    if (options.host === 'localhost' && options.port === 8100) {
        return '';
    }
    return '/mgmt';
}

function getRequest(requestOptions, responseHandler) {
    if (this.host === 'localhost' && this.port === 8100) {
        return this.http.request(requestOptions, responseHandler);
    }
    return this.https.request(requestOptions, responseHandler);
}
