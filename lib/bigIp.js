...
const REG_EXPS = require('./sharedConstants').REG_EXPS;
 ...
 * @param {String}  password                    - Password for user or URI (file, http, https, ARN, etc.)
 ...
 * @param {Boolean} [options.passwordIsToken]   - Indicates that the password is an auth token.
 * @param {Boolean} [options.passwordEncrypted] - Indicates that the password is encrypted (with
 *                                                the local cloud public key)
 * @param {Object} [options.retry]              - Specifies retries settings.
 ...
 * @param {Object}  [options.clOptions]         - Command line options
 *
...
BigIp.prototype.init = function init(host, user, password, options) {
    ...
    const authnOptions = {
        ...
        passwordIsToken: this.initOptions.passwordIsToken,
        passwordEncrypted: this.initOptions.passwordEncrypted,
        clOptions: this.initOptions.clOptions
    };
    ...
    return productPromise
        ...
        .then((icontrol) => {
            ...
            return this.ready(this.initOptions.retry || undefined);
        })
        .catch((err) => {
 ...
 * @param {String[]} [options.requiredFields]       - A list of expected fields. If any are missing, retry.
 *
    ...
    const func = function () {
        ...
        return isInitialized(this)
            ...
            .then((response) => {
                ...
                if (methodOptions.requiredFields) {
                    const isMissing = (requiredField) => {
                        const fieldIsMissing = !Object.hasOwnProperty.call(response, requiredField);
                        if (fieldIsMissing && !methodOptions.silent) {
                            this.logger.debug(`missing required field ${requiredField}`);
                        }
                        return fieldIsMissing;
                    };

                    if (methodOptions.requiredFields.some(isMissing)) {
                        this.logger.debug('missing required field');
                        return q.reject(new Error('missing required field'));
                    }
                }
                return response;
            });
...
BigIp.prototype.createOrModify = function createOrModify(path, body, iControlOptions, retryOptions, options) {
    ...
    // user objects handle partitions in their own way
    if (path === '/tm/auth/user' || path === '/tm/net/trunk') {
        partitionPath = '';
    } else if (body.partition) {
 ...
 *
 * @param {String}   hostname                       - Hostname of device
 * @param {Object}   [retryOptions]                 - Options for retrying the request.
 * @param {Integer}  [retryOptions.maxRetries]      - Number of times to retry if first try fails.
 *                                                    0 to not retry. Default 60.
 * @param {Integer}  [retryOptions.retryIntervalMs] - Milliseconds between retries. Default 10000.
 ...
 *                    or rejected if an error occurs.
 */
BigIp.prototype.deviceState = function deviceState(hostname, retryOptions) {
    const retry = retryOptions || util.DEFAULT_RETRY;

    const func = function () {
        return this.list(`/tm/cm/device/~Common~${hostname}`);
    };
...
BigIp.prototype.getPrivateKeyFilePath = function getPrivateKeyFilePath(folder, name) {
    ...
    return this.ready()
        ...
        .then((response) => {
            const KEY_FILE_PREFIX = `:${folder}:${name.replace(REG_EXPS.KEY_SUFFIX, '')}`;
...
BigIp.prototype.installPrivateKey = function installPrivateKey(privateKeyFile, folder, name, options) {
    ...
    const checkForKey = function checkForKey() {
        return this.list(`${CRYPTO_PATH}/~${folder}~${name}`);
    };
...
BigIp.prototype.getPrivateKeyMetadata = function getPrivateKeyMetadata(folder, name) {
    return this.ready()
        .then(() => {
            return this.list('/tm/sys/file/ssl-key');
        })
        .then((response) => {
            const keyPath = `/${folder}/${name}`;
            const keyPathNoSuffix = keyPath.replace(REG_EXPS.KEY_SUFFIX, '');
            const sslKeys = {};

            response.forEach((key) => {
                sslKeys[key.fullPath] = key;
            });
            // Look for specified key name, then the key name without the .key suffix
            if (sslKeys[keyPath]) {
                return q(sslKeys[keyPath]);
            } else if (sslKeys[keyPathNoSuffix]) {
                return q(sslKeys[keyPathNoSuffix]);
            }
            return q();
        });
};
 ...
 * @param {Boolean} [options.initLocalKeys]        - Re-create and install local public/private key pair
...
BigIp.prototype.loadUcs = function loadUcs(file, loadOptions, options) {
    ...
    return this.ready()
        ...
        .then(() => {
            if (restoreUser && !this.initOptions.passwordIsToken) {
                return util.runTmshCommand(`modify auth user ${this.user} password ${this.password}`);
            } else if (restoreUser && this.initOptions.passwordIsToken) {
                this.logger.info('Unable to restore user - token was used for initial authorization');
                return q();
            }
            return q();
        })
...
BigIp.prototype.ready = function ready(retryOptions) {
    ...
    const func = function () {
        ...
        if (this.isBigIp()) {
            availabilityChecks.push('/shared/iapp/package-management-tasks/available');
        }
        ...
        const readyCheck = function () {
            // perform ready check against /sys/ready endpoint
            // only available for BIG-IP 13.1+

            return this.deviceInfo()
                .then((deviceInfo) => {
                    const version = deviceInfo.version;

                    if (util.versionCompare(version, '13.1.0') < 0) {
                        return q.resolve({ skip: true });
                    }
                    return this.list('/tm/sys/ready', undefined, util.NO_RETRY);
                })
                .then((readyInfo) => {
                    if (readyInfo.skip) {
                        return q.resolve();
                    ...
                    const entries = readyInfo.entries['https://localhost/mgmt/tm/sys/ready/0']
                        .nestedStats.entries;
                    ...
                    return q.reject(new Error('System is not ready yet'));
                })
                .catch((err) => {
                    return q.reject(err);
                });
 ...
 * @param {String}  taskConfig                  - Body of the task request, typically JSON.
 * @param {Object}  [options]                   - Options controlling the task request.
 * @param {String}  [options.idAttribute]       - Name of the Task ID attribute in the task create response.
 *                                                  Default: '_taskId'
 * @param {Boolean} [options.validate]          - Whether to send a VALIDATING task request. Default: true
 * @param {Boolean} [options.neverReject]       - Whether to allow rejecting promise. Default: false.
 * @param {String}  [options.statusAttribute]   - Name of the Task Status attribute in the result response.
 *                                                  Default: '_taskState'
 *
 ...
 */
BigIp.prototype.runTask = function runTask(taskPath, taskConfig, options) {
    let taskId;
    return this.ready()
        .then(() => {
        ...
        .then((response) => {
            const idAttribute = (options && options.idAttribute) ? options.idAttribute : '_taskId';
            taskId = response[idAttribute];
        ...
        .then(() => {
            const checkOptions = {};
            if (options && options.statusAttribute) {
                checkOptions.statusAttribute = options.statusAttribute;
            }
            if (options && options.neverReject) {
                checkOptions.neverReject = options.neverReject;
            }
            return checkTask.call(this, taskPath, taskId, checkOptions);
        })
        .then((status) => {
            if (!status.success) {
                if (status.response && status.response.errorMessage) {
                    return q.reject(new Error(status.response.errorMessage));
                }
...
BigIp.prototype.saveUcs = function saveUcs(file) {
    return this.ready()
        .then(() => {
            ...
            return this.runTask(UCS_TASK_PATH, commandBody, { neverReject: true });
        })
        .then(() => {
        ...
        .then(() => {
            return util.runShellCommand(`gzip -t -v /var/local/ucs/${file}.ucs`);
        })
        .then((response) => {
            if (response.indexOf('NOT OK') !== -1) {
                return q.reject(new Error('Validation of generated UCS file failed; ' +
                    'newly generated UCS file appears to be corrupted'));
            }
            return q.resolve();
        })
        .catch((err) => {
...
/**
 * Specifies the management ip. This does not set the ip on the device, just specifies
 * the ip that the device is currently using
 *
 * @param {String} host - host (ip) to use for management IP
 *
 * @returns {Promise} A promise which is resolved when the operation is complete
 *                    or rejected if an error occurs.
 */
BigIp.prototype.setHost = function setHost(host) {
    this.host = host;
    this.icontrol.host = (host === 'localhost' ? '127.0.0.1' : host);
...
/**
 * Specifies the management ip port. This does not set the port on the device, just specifies
 * the port that the device is currently using
 *
 ...
 * @param {Object} [options]                    - Options for parsing task response
 * @param {String} [options.statusAttribute]    - Name of the Task Status attribute in the result response.
 *                                                  Default: '_taskState'
 *
 * @returns {Promise} A promise which is resolved with true if the task completes
 ...
 *                    or rejected if some other error occurs.
 */
function checkTask(taskPath, taskIdToCheck, options) {
    const retryOptions = util.DEFAULT_RETRY;
    // fall back to checking for success in some other way if task ID is lost
    // when REST services restart during UCS restore, for example
    retryOptions.failOnErrorMessages = ['Task not found'];

    const func = function () {
        const deferred = q.defer();
        this.list(`${taskPath}/${taskIdToCheck}`, undefined, util.SHORT_RETRY)
            .then((response) => {
                const statusAttribute = (options && options.statusAttribute)
                    ? options.statusAttribute
                    : '_taskState';
                const taskState = response[statusAttribute];
                if (taskState === 'VALIDATING' || taskState === 'STARTED' || taskState === 'CREATED') {
                    // this is a normal state, just not done yet - keep waiting
                    deferred.reject();
                } else if (taskState === 'COMPLETED' || taskState === 'FINISHED') {
                    deferred.resolve({ success: true, response });
                } else if (taskState === 'FAILED') {
                    deferred.resolve({ success: false, response });
                } else {
...
/**
 * Checks for matching text in log file
 *
 * @param {String} logPath                     - URL of the log file to check
 * @param {String} textToCheck                 - Text to check for in the log file
 * @param {Object} [options]                   - Options for parsing the response
 *
 * @returns {Promise} A promise which is resolved with true if the text is found,
 *                    resolved with false if task goes to error state
 *                    or rejected if some other error occurs.
 */
function checkLogs(logPath, textToCheck, options) {
    const func = function () {
        const deferred = q.defer();
        this.list(`${logPath}/`, undefined, util.SHORT_RETRY)
            .then((response) => {
                if (response.apiRawValues.apiAnonymous.indexOf(textToCheck) !== -1) {
                    deferred.resolve();
                } else {
                    deferred.reject(new Error(`checkLogs: could not find ${textToCheck} in ${logPath}`));
                }
            })
            .catch((err) => {
                if (options && options.neverReject) {
...
function restorePrimaryKey(ucsFile) {
    const logId = 'restorePrimaryKey:';
    ...
    return util.runShellCommand('bigstart stop')
        ...
        .then(() => {
            ...
            return util.runShellCommand(
                `tar --warning=no-timestamp -xf ${ucsFile} -C ${tempUcsDir}`
            );
        })
        .then(() => {
            this.logger.silly(logId, 'untar success, reading key');
            return util.readDataFromFile(`${tempUcsDir}${BACKUP.PRIMARY_KEY_DIR}`);
        })
        .then((oldPrimaryKey) => {
            this.logger.silly(logId, 'read success, writing key');
            return util.writeDataToFile(oldPrimaryKey, BACKUP.PRIMARY_KEY_DIR);
        })
        .then(() => {
            this.logger.silly(logId, 'wrote primary key, reading unit key');
            return util.readDataFromFile(`${tempUcsDir}${BACKUP.UNIT_KEY_DIR}`);
        })
        .then((oldUnitKey) => {
            this.logger.silly(logId, 'read unitkey success, writing unit key');
            return util.writeDataToFile(oldUnitKey, BACKUP.UNIT_KEY_DIR);
        })
        .then(() => {
        ...
        .catch((err) => {
            this.logger.debug('error restoring primary key', err && err.message ? err.message : err);
            return q.reject(err);
        });
}
...
function runLoadUcs(file, ucsLoadOptions) {
    ...
    this.runTask(UCS_TASK_PATH, commandBody)
        ...
        .catch((err) => {
            ...
            this.logger.silly('checking if UCS restored successfully');
            checkLogs.call(this, '/tm/sys/log/ltm/stats', 'UCS installation success')
                .then(() => {
                    this.logger.silly('successfully restored UCS after task not found');
                    deferred.resolve();
                })
                .catch(() => {
                    restorePrimaryKey.call(this, file)
                        .then(() => {
                            this.logger.silly('primary key restored, waiting for ready');
                            return this.ready();
                        })
                        .then(() => {
                            this.logger.silly('big ip ready after primary key restore');
                            deferred.resolve();
                        })
                        .catch((copyErr) => {
