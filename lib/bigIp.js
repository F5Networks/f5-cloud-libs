...
/**
 * Using the management-ip address, determine and return the true MAC Address
 *
 * @returns {Promise}   If successful, it should resolve with the macAddress for management interface.
 *                      If unsuccessful, it should reject with the error.
 */
BigIp.prototype.getManagementMac = function getManagementMac() {
    return this.ready()
        .then(() => {
            return this.list('/tm/net/interface/mgmt', undefined, util.SHORT_RETRY);
        })
        .then((response) => {
            return q.resolve(response.macAddress);
        });
};
