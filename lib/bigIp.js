...
var getIControl = function(host, user, password) {
    return new IControl({
        host: host,
        user: user,
        pass: password,
...
function BigIp(host, user, password, testOpts) {
    ...
    icontrol = testOpts.icontrol || getIControl(host, this.user, password);
}
 ...
 */
BigIp.prototype.password = function(user, newPassword, oldPassword) {
    var deferred = q.defer();
    var promise;
    ...
    this.ready()
        .then(function() {
            ...
            promise
                .then(function(response) {
                    // If we're setting the password for our user, we need to update
                    // our icontrol
                    if (user === this.user) {
                        icontrol = getIControl(this.host, this.user, newPassword);
                    }
                    deferred.resolve(response);
                }.bind(this))
                .catch(function(err) {
