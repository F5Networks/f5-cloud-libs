...
BigIp.prototype.ready = function(maxRetries, retryIntervalMs) {
    ...
    var isReady = function() {
        ...
        icontrol.list('/shared/echo-js', function(err, response) {
            if (err) {
                ...
                if (!response.stage) {
                    deferred.reject('No stage in echo-js response');
                }
                else if (response.stage !== 'STARTED') {
                    deferred.reject('echo-js stage not STARTED yet');
                }
                else {
                   deferred.resolve();
                }
    ...
    return util.tryUntil(maxRetries, retryIntervalMs, isReady);
};
...
BigIp.prototype.initialSetup = function(options) {
    ...
    var executeCommands = function() {
        ...
        if (commands.length > 0) {
            ...
            return q("Initial setup complete.");
        }
    }.bind(this);
    ...
    if (options.hostname) {
        return this.list('/tm/cm/device')
            .then(function(response) {
                var oldName = response[0].hostname;
