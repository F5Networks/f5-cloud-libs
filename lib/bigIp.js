...
const CLOUD_LIBS_PRIVATE_KEY = 'cloudLibsPrivate';
const CLOUD_LIBS_PRIVATE_KEY_FOLDER = 'CloudLibs';
...
/**
 * Gets the path to the latest cloud private key we have installed
 *
 * @returns {Promise} A promise which is resolved when the request is complete
 *                    or rejected if an error occurs.
 */
BigIp.prototype.getCloudPrivateKeyFilePath = function() {

    const PRIVATE_KEY_DIR = '/config/filestore/files_d/' + CLOUD_LIBS_PRIVATE_KEY_FOLDER + '_d/certificate_key_d/';
    ...
    return this.ready()
        ...
        .then(function(response) {
            const KEY_FILE_PREFIX = ':' + CLOUD_LIBS_PRIVATE_KEY_FOLDER + ':' + CLOUD_LIBS_PRIVATE_KEY + '.key';
 ...
 *                    or rejected if an error occurs.
 */
BigIp.prototype.installCloudPrivateKey = function(privateKeyFile, options) {
    ...
    var installBody = {
        command: 'install',
        name: '/' + CLOUD_LIBS_PRIVATE_KEY_FOLDER + '/' + CLOUD_LIBS_PRIVATE_KEY,
    ...
    var checkForKey = function() {
        return this.list(CRYPTO_PATH + '/~' + CLOUD_LIBS_PRIVATE_KEY_FOLDER + '~' + CLOUD_LIBS_PRIVATE_KEY + '.key');
    };

    this.ready()
        .then(function() {
            return this.createFolder(CLOUD_LIBS_PRIVATE_KEY_FOLDER, {subPath: '/'});
        }.bind(this))
        .then(function() {
...
/**
 * Get the data for the cloud libs private key
 *
 * @returns {Promise} A promise which is resolved when the request is complete
 *                    or rejected if an error occurs.
 */
BigIp.prototype.getCloudPrivateKey = function() {
    return this.ready()
        .then(function() {
            return this.list('/tm/sys/file/ssl-key/~' + CLOUD_LIBS_PRIVATE_KEY_FOLDER + '~' + CLOUD_LIBS_PRIVATE_KEY + '.key');
        }.bind(this));
};
