    ...
    const func = function () {
        return isInitialized(this)
            .then(() => {
                ...
                this.icontrol.list(`${path}/${body.name}`)
                    .then(() => {
                        if (!methodOptions.silent) {
                            finalPath = `${path}/${body.name}`;
                            this.logger.silly(`${path}/${body.name} exists, modifying`);
                    ...
                    .catch((err) => {
                        if (err.code === 404) {
                            if (!methodOptions.silent) {
                                this.logger.silly(`${path}/${body.name} does not exist, creating`);
