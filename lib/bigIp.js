 ...
 *
 * @param {String} name - Name of the key without the '.key' suffix
 *
 * @returns {Promise} A promise which is resolved when the request is complete
 *                    or rejected if an error occurs.
 */
BigIp.prototype.getCloudPrivateKeyFilePath = function(name) {

    const PRIVATE_KEY_DIR = '/config/filestore/files_d/Common_d/certificate_key_d/';
    ...
    return this.ready()
        ...
        .then(function(response) {
            const KEY_FILE_PREFIX = ':Common:' + name + '.key';
 ...
 * @param {String} name           - Name for the key. '.key' is appended by TMOS.
 *
 * @returns {Promise} A promise which is resolved when the request is complete
 *                    or rejected if an error occurs.
 */
BigIp.prototype.installCloudPrivateKey = function(privateKeyFile, name) {
    ...
    var checkForKey = function() {
        return this.list(CRYPTO_PATH + '/~Common~' + name + '.key');
    };
