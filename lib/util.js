...
module.exports = {
    ...
    /**
     * Creates a ramdom on BIG-IP/BIG-IQ.
     *
    ...
    createRandomUser() {
        ...
        return cryptoUtil.generateRandomBytes(USER_NAME_LENGTH, 'hex')
            .then((data) => {
                user = data;

                // Get a random password for the user
                return cryptoUtil.generateRandomBytes(USER_PASSWORD_LENGTH, 'base64');
            })
            .then((data) => {
                password = data;
                return this.runTmshCommand(
            ...
            .catch((err) => {
                logger.info('Failed to create user', err && err.message ? err.message : err);
                return q.reject(err);
            });
    },
    ...
    readData(dataOrUri, dataIsUri, options) {
        ...
        if (dataIsUri) {
            ...
            if (dataOrUri.startsWith('arn')) {
                // AWS arn
                const provider = cloudProviderFactory.getCloudProvider('aws', options);
                ...
                provider.init()
                    ...
                    .catch((err) => {
                        logger.info('Could not find BIG-IQ credentials file on the specified S3 bucket');
                        deferred.reject(err);
                    });
            } else if (dataOrUri.match(azureRegex)) {
                // Azure blob
                const provider = cloudProviderFactory.getCloudProvider('azure', options);

                provider.init(options.clOptions)
                    .then(() => {
                        return this.tryUntil(
                            provider,
                            this.MEDIUM_RETRY,
                    ...
                    .catch((err) => {
                        logger.info('Could not find BIG-IQ credentials file in Azure Storage');
                        deferred.reject(err);
                    });
            } else {
                // Plain old url
    ...
    */
    getProductString() {
        return this.runShellCommand('grep -m 1 product /config/bigip_base.conf | awk \'{print $2}\'')
            .then((response) => {
