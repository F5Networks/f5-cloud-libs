...
module.exports = {
            ...
            fs.stat(REBOOT_SCRIPTS_DIR, function(err) {
                if (err && err.code === 'ENOENT') {
                    try {
                        ...
                        if (err.code !== 'EEXIST') {
                            this.logger.error("Error creating", REBOOT_SCRIPTS_DIR, "Not saving args for a second try.", err);
                            deferred.resolve();
                        }
                    }
                }
                ...
                try {
                    ...
                    this.logger.warn("Unable to open args file", err);
                    deferred.resolve();
                }
            }.bind(this));
        }
        catch (err) {
            this.logger.warn("Unable to stat", REBOOT_SCRIPTS_DIR);
            deferred.resolve();
        }
    ...
    /**
     * Gets a password from a URL.
     *
     * Only file URL is supported for now.
     *
     * @param {String} url - URL from which to get the password. Only
     *                       file URL is supported for now.
     *
     * @returns {String} The password.
     */
    getPasswordFromUrl: function(url) {
        var URL = require('url');
        var fs = require('fs');
        ...
        if (parsedUrl.protocol === 'file:') {
            password = fs.readFileSync(parsedUrl.pathname, {encoding: 'ascii'});
            return password.trim();
        }
        else {
            throw new Error('Only file URLs are currently supported.');
        }
    },
