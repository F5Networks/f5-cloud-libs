...
module.exports = {
    ...
    getProduct() {
        return this.getProductString()
            .then((response) => {
                if (!response) {
                    this.runTmshCommand('save sys config')
                        .then(() => {
                            return this.getProductString()
                                .then((product) => {
                                    return q(product);
                                ...
                                .catch((productErr) => {
                                    return q.reject(productErr);
                                });
                        })
                        .catch((err) => {
                            logger.silly('Unable to save sys config', err && err.message ? err.message : err);
                            return q.reject(err);
                        });
                } else {
                    return q(response);
            ...
            .catch((productStringErr) => {
                return q.reject(productStringErr);
            });
    },
