...
module.exports = {
        ...
        var _tryUntil = function(maxRetries, interval, funcToTry, deferred) {
            ...
            try {
                promise = funcToTry.apply(thisArg, args)
                    ...
                    .catch(function(err) {
                        logger.verbose('tryUntil error:', err ? (err.message ? err.message : err) : '', 'tries left:', maxRetries);
                        retryOrReject(err);
                    }.bind(this));
    ...
    map: function(mapString, container) {
        ...
        for (i = 0; i < params.length; ++i) {
            container[params[i].trim()] = params[i + 1].trim();
            i++;
        }
    },
     ...
     */
    mapArray: function(mapString, container) {
        var mapObject = {};
        var params;
        var i;
     ...
     */
    writeDataToUrl: function(data, url) {
        var URL = require('url');
        var deferred = q.defer();
        var parsedUrl;
     ...
     *                   or rejected if an error occurs.
     */
    getDataFromUrl: function(url, options) {
        var URL = require('url');
        var fs = require('fs');
        ...
        try {
            if (parsedUrl.protocol === 'file:') {
                fs.readFile(parsedUrl.pathname, {encoding: 'ascii'}, function(err, data) {
                    if (err) {
                        deferred.reject(err);
                    }
                    else {
                ...
                executor.get(requestOptions, function(response) {
     ...
     *                    command or rejected if an error occurs.
     */
    runTmshCommand: function(command) {
        var tmshCommand = '/usr/bin/tmsh ' + command;
