...
module.exports = {
    ...
    /**
     * Decrypts a secret, typically a password
     *
     * @static
     *
    ...
    decryptPassword(password) {
        ...
        return localKeyUtil.getPrivateKeyFilePath(KEYS.LOCAL_PRIVATE_KEY_FOLDER, KEYS.LOCAL_PRIVATE_KEY)
            .then((response) => {
                ...
                return localKeyUtil.getPrivateKeyMetadata(
    ...
    decryptDataFromRestStorage(id, salt) {
        const cryptoUrl = 'http://localhost:8100/shared/restricted-store/crypto';
        const headers = {
        ...
        return httpUtil.post(cryptoUrl, { headers, body })
            .then((data) => {
                return data.data;
            })
            .catch((err) => {
