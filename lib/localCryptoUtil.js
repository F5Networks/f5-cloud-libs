...
module.exports = {
    ...
    decryptData(data, privateKeyFolder, privateKeyName, options) {
        ...
        return localKeyUtil.getExistingPrivateKeyName(privateKeyFolder, privateKeyName)
            .then((response) => {
                existingPrivateKeyName = response;
                return localKeyUtil.getPrivateKeyFilePath(privateKeyFolder, existingPrivateKeyName);
            })
            .then((privateKeyFilePath) => {
                ...
                return localKeyUtil.getPrivateKeyMetadata(privateKeyFolder, existingPrivateKeyName);
            })
            .then((metadata) => {
                if (!metadata) {
    ...
    /**
     * Decrypts a secret that was encrypted symmetrically, typically a large password
     * or object (JSON) with multiple values that was encrypted with our local private keys.
     *
     * This is just a shortcut for {@link decryptData} with symmetric options
     *
     * @static
     *
     * @param {Object} data - Object (JSON) created by symmetric encrypt operation
     *
     * @returns {Promise} A promise which is resolved with the decrypted
     *                    secret or rejected if an error occurs
     */
    symmetricDecryptPassword(data) {
        assert.ok(data, 'data is required');
        ...
        return this.decryptData(
            ...
            parsedData.privateKey.folder,
            parsedData.privateKey.name,
            {
                encryptedKey: parsedData.encryptedKey,
                iv: parsedData.iv
            }
        );
    },
     ...
     * @param {Object}  [options]           - Optional parameters
     * @param {Boolean} [options.symmetric] - Data was symmetrically encrypted
     *
     * @returns {Promise} A promise which is resolved with the decrypted data or
     *                    rejected if an error occurs.
     */
    decryptDataFromFile(dataFile, options) {
        let dataToDecrypt;
        ...
        return util.readDataFromFile(dataFile)
            .then((data) => {
                ...
                if (options && options.symmetric) {
                    return this.symmetricDecryptPassword(dataToDecrypt);
                }
                return this.decryptPassword(dataToDecrypt);
            })
            .catch((err) => {
