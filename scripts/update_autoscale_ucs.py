...
def main():
    ...
    if not original_ucs or not cloud_provider:
        print "Usage: "
        print ("    ./%s --cloud-provider <aws | azure | gce> --original-ucs <ucs_filename>" % (sys.argv[0]))
        print ("    ./%s --cloud-provider <aws | azure | gce> --original-ucs <ucs_filename> --updated-ucs <ucs_filename>" % (sys.argv[0]))
        print "ex. "
        print ("    ./%s --cloud-provider aws --original-ucs original.ucs --updated-ucs updated.ucs" % (sys.argv[0]))
        sys.exit()
    ...
    try:
        ...
        sys.exit("I/O Error({0}): {1} - If the latest UCS file is corrupted and/or the current primary host is stuck at BECOMING_PRIMARY state, the corrupted UCS file needs to be deleted as well as primary host needs to be terminated to allow re-elect the new primary.'".format(e.errno, e.strerror))
    ...
    # Remove the cloud directory created by the template so we don't overwrite new code with old
    path_to_exclude = "/config/cloud/" + cloud_provider
